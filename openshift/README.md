...
$ oc new-app zk-builder -p GITHUB_REF="v3.4.13" IMAGE_STREAM_VERSION="3.4.12"
```
...
zookeeper   172.30.1.1:5000/myproject/zookeeper   3.4.13    1 days ago
```

3 - If you want to use this local/private image from containers on other projects then use the "\<project\>/NAME" value as `SOURCE_IMAGE` parameter value, and use one value of "TAGS" as `ZOO_VERSION` parameter value (e.g: myproject/zookeeper:3.4.13).
...
$ oc new-app zk -p ZOO_REPLICAS=1 [-p SOURCE_IMAGE="172.30.1.1:5000/myproject/zookeeper:3.4.13"]
```
